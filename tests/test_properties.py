...
from core.viscosity import test_viscosity_calculations, screen_fragments_by_viscosity
...
def run_all_tests():
    ...
    print("\n" + "="*50)
    print("Testing Viscosity Calculations")
    print("="*50)
    test_viscosity_calculations(fragments_data, combinations)
    ...
    visc_range = (0.0005, 0.1)  # Pa·s (typical range for low-viscosity ILs)
    ...
    visc_results = screen_fragments_by_viscosity(fragments_data, visc_range)
    ...
    for frag_type in fragments_data.keys():
        ...
        visc_names = {f['name'] for f in visc_results[frag_type]}
        
        # Find fragments that meet all criteria
        common_fragments = density_names & cp_names & tox_names & visc_names
        ...
        if common_fragments:
            for name in common_fragments:
                ...
                visc_frag = next((f for f in visc_results[frag_type] if f['name'] == name), None)
                
                if all([density_frag, cp_frag, tox_frag, visc_frag]):
                    ...
                    print(f"    Viscosity: {visc_frag.get('estimated_viscosity', 'N/A'):.6f} Pa·s")
        else:
